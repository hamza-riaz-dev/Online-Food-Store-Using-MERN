...
export default function Card(props) {
    ...
    let unit = [props.prodItem.unitS, props.prodItem.unitL];
    ...
    const addToCart = async () => {
        ...
        if (prod !== []) {
            if (prod.size === size) {
                await dispatch({ type: "UPDATE", id: props.prodItem._id, price: finalPrice, qty: qty });
                return
            }


            else if (prod.size !== size) {
                await dispatch({ type: "ADD", id: props.prodItem._id, name: props.prodItem.name, img: props.prodItem.img, qty: qty, size: size, price: finalPrice });
                return
            }
            return
        }
    ...
    return (
        <>
            <div className="card m-3" style={{ "width": "18rem", "maxHeight": "500px" }}>
                <img src={props.prodItem.img} className="card-img-top" alt="..." style={{ height: "200px", objectFit: "fill" }} />
                <div className="card-body">
                    ...
                    <div className='container w-100'>
                        ...
                        <hr />
                        <button className='btn btn-dark justify-center ms-2' onClick={addToCart}>Add to cart</button>
                    </div>
                </div>
            </div>
        </>
